...
export const validateBudgetAllocation = [
  (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!allocatedAmount || isNaN(allocatedAmount)) errors.push({ field: 'allocatedAmount', message: 'Allocated amount must be a number' });
    if (!description) errors.push({ field: 'description', message: 'Description is required' });
    if (!period) errors.push({ field: 'period', message: 'Period is required' });
...
export const validateExpense = [
  (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!category) errors.push({ field: 'category', message: 'Category is required' });
    if (!amount || isNaN(amount)) errors.push({ field: 'amount', message: 'Amount must be a number' });
    if (!date) errors.push({ field: 'date', message: 'Valid date is required' });
    if (!description) errors.push({ field: 'description', message: 'Description is required' });
    if (!employeeId) errors.push({ field: 'employeeId', message: 'Valid employee ID is required' });
...
export const validateCostCenter = [
  (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!description) errors.push({ field: 'description', message: 'Description is required' });
    if (!manager) errors.push({ field: 'manager', message: 'Manager is required' });
    if (!budget || isNaN(budget)) errors.push({ field: 'budget', message: 'Budget must be a number' });
...
export const checkProjectFinanceAccess = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { projectId } = req.params;
...
export const checkBudgetLimits = async (req: Request, res: Response, next: NextFunction) => {
  try {
