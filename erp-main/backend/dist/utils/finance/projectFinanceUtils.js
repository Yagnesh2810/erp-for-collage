...
exports.ProjectFinanceUtils = void 0;
class ProjectFinanceUtils {
    // Calculate budget utilization percentage
    static calculateBudgetUtilization(spent, budget) {
        if (budget <= 0)
            return 0;
        return Math.round((spent / budget) * 100);
    }
    // Calculate variance between budgeted and actual amounts
    static calculateVariance(actual, budgeted) {
    ...
    static calculateBurnRate(expenses, periodDays = 30) {
    ...
    static calculateRunway(remainingBudget, burnRate) {
        if (burnRate <= 0)
            return Infinity;
    ...
    static generateAccountCode(type, category) {
        ...
        if (type === 'expense' || type === 'revenue') {
            return codes[type][category] ||
                codes[type]['Other'];
        }
        return codes[type] || '9999';
    }
    // Generate reference number for transactions
    static generateReferenceNumber(type, projectId) {
    ...
    static validateBudgetAllocation(allocation, projectBudget, existingAllocations) {
        const errors = [];
    ...
    static calculateCostCenterEfficiency(costCenter) {
    ...
    static calculateFinancialHealthScore(project) {
        let score = 100;
        const factors = [];
...
exports.ProjectFinanceUtils = ProjectFinanceUtils;
