...
exports.getDoubleEntryAudit = exports.validateDoubleEntry = exports.createFromTemplate = exports.getTransactionTemplates = void 0;
...
const validateDoubleEntry = async (req, res) => {
    try {
            ...
            if (line.accountId) {
                ...
                if (!account) {
                    ...
                    if ((account.normalBalance === 'debit' && credit > debit) ||
                        ...
                        validation.warnings.push(`Line ${i + 1}: Entry opposite to account's normal balance (${account.normalBalance})`);
                    }
                }
            }
        ...
        if (difference > 0.01) {
            validation.isValid = false;
            validation.errors.push(`Debits (${validation.summary.totalDebit}) must equal Credits (${validation.summary.totalCredit}). Difference: ${difference}`);
        }
        validation.summary.isBalanced = validation.isValid && difference < 0.01;
        res.json(validation);
    }
    catch (error) {
        res.status(500).json({
...
const getDoubleEntryAudit = async (req, res) => {
    try {
        ...
        for (const entry of entries) {
            ...
            const entryAudit = {
                ...
                lines: entry.lines.map(line => ({
                    account: {
                        code: line.accountId.code,
                        name: line.accountId.name,
                        type: line.accountId.type
                    },
                    debit: line.debit,
                    credit: line.credit,
        ...
        audit.summary.systemIsBalanced =
...
exports.getDoubleEntryAudit = getDoubleEntryAudit;
