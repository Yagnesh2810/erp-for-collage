    ...
    try {
        ...
        for (const data of productData) {
            const product = await Product.create({
                ...
                stock: data.stock,
                image: `/images/products/${data.sku.toLowerCase()}.jpg`,
                isActive: true
            });
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const status = i < 3 ? 'completed' : i < 6 ? 'in-progress' : i < 8 ? 'planning' : 'on-hold';
            const progress = status === 'completed' ? 100 : status === 'in-progress' ? Math.floor(Math.random() * 70) + 20 : Math.floor(Math.random() * 20);
        ...
        for (const project of projects) {
            ...
            for (let i = 0; i < numTasks; i++) {
                ...
                const assignee = project.teamMembers[Math.floor(Math.random() * project.teamMembers.length)];
